...
1. If not found: deny the call, log as `tool_not_in_catalog`.
1. Extract `server.url`, `server.tls_fingerprint`, `server.spiffe_id`, and `server.rotation_mode` from the catalog entry.
1. Open (or reuse) a connection to `server.url`.
1. Verify the server's identity:
1. If `tls_fingerprint` is set: verify the server's TLS certificate public key fingerprint matches `catalog[tool_name].server.tls_fingerprint`. For `key-pinned` mode, the fingerprint is stable across cert renewals; for `cert-pinned` mode, a new cert triggers an identity mismatch even if the key is the same.
1. If `spiffe_id` is set: verify the server's SPIFFE SVID matches `catalog[tool_name].server.spiffe_id` via the SPIRE trust bundle.
1. If either verification that is configured fails: deny the call, log as `identity_mismatch`.
1. Fetch the live tool definition from the upstream server and compare against `approved_definition` using `definition_hash`.
1. If mismatch: deny the call, log as `definition_mismatch`.
1. Forward the tool call to the upstream server.
1. Return the response to the runtime's response inspection pipeline.
...
**Mode A: Key-pinning (recommended)** The catalog entry stores the fingerprint of the server's current public key. The server reuses the same keypair at cert renewal (renewing only the certificate, not the key). The fingerprint does not change at renewal. No catalog update required.
...
**Mode B: Cert-pinning** The catalog entry stores the fingerprint of the server's specific certificate (including its expiry). At cert renewal (even with the same keypair), the fingerprint changes. A catalog update and enclave restart are required.
