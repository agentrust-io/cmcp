...
| `POLICY_SIGNING_KEY_INVALID` | 500 | FATAL | `CMCP_POLICY_SIGNING_KEY` or `CMCP_POLICY_SUCCESSOR_SIGNING_KEY` is not a 32-byte Ed25519 public key, the two are equal, or a successor is set without a current key | [configuration.md](../configuration.md) |
| `POLICY_SIGNING_KEY_REVOKED` | 403 / startup | FATAL at startup, WARN at runtime | A policy bundle verifies only under a revoked policy signing key and is refused; or the policy in force was signed by a key revoked since it was installed, so every tool call is refused in every enforcement mode until a bundle signed by a trusted key is installed; or every pinned key is revoked | [policy-hot-reload.md](policy-hot-reload.md#revocation-without-a-restart) |
| `POLICY_KEY_REVOCATION_INVALID` | none (logged) | WARN | A statement in `signing-key-revocations.json` was refused: malformed, not signed by the current or successor key, or naming a key other than the current one. It is skipped; other statements and the bundle reload still proceed | [policy-hot-reload.md](policy-hot-reload.md#revocation-without-a-restart) |
