...
As noted in [threat-model.md](https://cmcp.agentrust-io.com/docs/spec/threat-model/index.md), T.1 (server swap / tool identity) is only closed if the agent or the agent's runtime runs `verify_trace_claim` before sending traffic. Attestation without verification is post-hoc evidence, not a runtime gate.
