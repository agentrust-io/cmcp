...
What is measured is the installed distributions' recorded per-file hashes (pip's `RECORD` metadata), plus the policy bundle bytes and the resolved configuration with secrets excluded. RECORD covers dependencies, so a swapped transitive package is caught; measuring only cMCP's own source would report an identical digest for it. An editable install has no RECORD, so the measurement is refused rather than computed over an unknown subset: fatal in production, a warning under `CMCP_DEV_MODE`. Each file RECORD lists with a hash is read and hashed at measurement time, and a file whose bytes no longer match RECORD refuses the measurement the same way, so editing an installed file without editing RECORD does not leave the digest unchanged. A listed file that has been deleted is marked `<missing>` in the digest. Files RECORD lists without a hash (bytecode caches, RECORD itself) and files RECORD does not list at all are not covered. The collector runs inside the environment it measures, so this detects modification by anything that does not also control the collector; it does not defend against an attacker who does.
